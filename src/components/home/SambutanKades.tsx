...
import { Building2, UserCheck, Quote, Landmark } from "lucide-react";
...
export default function SambutanKades({ profilData, listPerangkat }: SambutanKadesProps) {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 sm:mb-16">
      <Card className="p-6 sm:p-8 lg:p-10 shadow-xs border-border/80 relative overflow-hidden bg-card">
        {/* Subtle decorative background glow */}
        <div className="absolute top-0 right-0 w-80 h-80 bg-emerald-50/50 dark:bg-emerald-950/20 rounded-full blur-3xl pointer-events-none -mr-20 -mt-20" />

        <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-6 sm:gap-8 lg:gap-10">
          {/* Portrait Photo Frame */}
          <div className="w-44 sm:w-52 md:w-56 aspect-[3/4] rounded-2xl overflow-hidden bg-muted border border-border shadow-xs flex-shrink-0 relative group flex items-center justify-center">
            <ImageWithSkeleton
              ...
              fallbackIcon={<UserCheck className="w-14 h-14 text-muted-foreground" />}
            />

            {/* Official Badge Pill */}
            <div className="absolute top-2.5 left-2.5 z-10">
              <Badge className="bg-[#063321]/90 backdrop-blur-sm text-white text-[10px] font-bold px-2.5 py-1 rounded-full shadow-xs gap-1 border-emerald-500/30">
          ...
          <div className="flex-grow space-y-4 text-center md:text-left flex flex-col justify-between self-stretch">
            <div className="space-y-3">
              {/* Header Label */}
              <div className="inline-flex items-center space-x-1.5 text-xs font-bold text-emerald-800 dark:text-emerald-400 uppercase tracking-wider">
                <Quote className="w-3.5 h-3.5 text-emerald-600 flex-shrink-0" />
              ...
              {/* Sincere Quote Text */}
              <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed font-normal italic">
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-emerald-800 dark:text-emerald-400 font-medium mt-0.5 flex items-center justify-center md:justify-start gap-1">
                ...
                <Badge
                  variant="secondary"
                  className="gap-1.5 py-1 px-3 self-center md:self-auto text-[11px] font-medium"
                >
